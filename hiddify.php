...
function getdatauser($username, $location)
{
    ...
    $url = $durwinam_list_get['url_panel'] . '/api/v2/admin/user/';
    $ch = curl_init();
    ...
    curl_setopt($ch, CURLOPT_HTTPHEADER, array(
        'Authorization: Basic ' . base64_encode("{$durwinam_list_get['secret_code']}:")
    ));
...
{
    $durwinam_list_get = select("durwinam_panel", "*", "name_panel", $location, "select");
    $url = $durwinam_list_get['url_panel'] . '/api/v2/admin/server_status/';
    $headers = array(
        'Authorization: Basic ' . base64_encode("{$durwinam_list_get['secret_code']}:")
    );
...
function adduserhi($location, array $data)
{
    ...
    $headers = array(
        ...
        'Hiddify-API-Key: ' . $durwinam_list_get['secret_code']
    );
    $req = new CurlRequest($url);
    $req->setHeaders($headers);
...
function updateuserhi($username, $location, array $data)
{
    ...
    $url = $durwinam_list_get['url_panel'] . '/api/v2/admin/user/' . $paneldata['uuid'] . "/";
    $payload = json_encode($data, true);
    $headers = array(
        'Accept: application/json',
        'Content-Type: application/json',
...
{
    $durwinam_list_get = select("durwinam_panel", "*", "name_panel", $location, "select");
    $url = $durwinam_list_get['url_panel'] . "/api/v2/admin/user/$uuid/";
    $headers = array(
        'Accept: application/json',
        'Content-Type: application/json',
        'Hiddify-API-Key: ' . $durwinam_list_get['secret_code']
