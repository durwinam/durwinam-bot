...
function DirectPayment($order_id, $image = 'images.jpg')
{
    ...
    if ($steppay[0] == "getconfigafterpay") {
        ...
        $durwinam_list_get = select("durwinam_panel", "*", "name_panel", $get_invoice['Service_location'], "select");
        ...
        if ($dataoutput['username'] == null) {
            ...
            $texterros = sprintf($textbotlang['Admin']['reportgroup']['errorConfigCreate'], $dataoutput['msg'], $Balance_id['id'], $Balance_id['username'], $durwinam_list_get['name_panel']);
        ...
        sendMessageService($durwinam_list_get, $dataoutput['configs'], $output_config_link, $dataoutput['username'], $Shoppinginfo, $textcreatuser, $get_invoice['id_invoice'], $get_invoice['id_user'], $image);
        ...
        if ($extend['status'] == false) {
            ...
            $textreports = sprintf($textbotlang['Admin']['reportgroup']['errorRenewServiceFn'], $durwinam_list_get['name_panel'], $nameloc['username'], $extend['msg']);
        ...
        $durwinam_list_get = select("durwinam_panel", "*", "name_panel", $nameloc['Service_location'], "select");
        $Balance_Low_user = 0;
        $inboundid = $durwinam_list_get['inboundid'];
        ...
        $dateacc = date('Y/m/d H:i:s');
        $type = "extra_user";
        $extra_volume = $ManagePanel->extra_volume($nameloc['username'], $durwinam_list_get['code_panel'], $volume);
        if ($extra_volume['status'] == false) {
            $extra_volume['msg'] = json_encode($extra_volume['msg']);
            $textreports = sprintf($textbotlang['Admin']['reportgroup']['errorExtraVolumeFn'], $durwinam_list_get['name_panel'], $nameloc['username'], $extra_volume['msg']);
        ...
        $extra_time = $ManagePanel->extra_time($nameloc['username'], $durwinam_list_get['code_panel'], $tmieextra);
        if ($extra_time['status'] == false) {
            $extra_time['msg'] = json_encode($extra_time['msg']);
            $textreports = sprintf($textbotlang['Admin']['reportgroup']['errorExtraTimeFn'], $durwinam_list_get['name_panel'], $nameloc['username'], $extra_time['msg']);
...
{
    global $from_id, $optionDurwinam, $optionX_ui_single, $optionhiddfy, $option_durwinam, $optionalireza_single, $optionmarzneshin, $option_mikrotik, $optionwg, $options_ui, $optionibsng, $optionrebecca;
    if ($typepanel == "durwinam") {
        sendmessage($from_id, $message, $optionDurwinam, 'HTML');
    ...
    } elseif ($typepanel == "durwinam_agent") {
        sendmessage($from_id, $message, $option_durwinam, 'HTML');
...
{
    $dir = sys_get_temp_dir() . DIRECTORY_SEPARATOR . 'durwinambot_qr';
