...
precheck_fresh_server() {
    ...
    # Known VPN panels
    { [ -d /opt/durwinam ] || [ -d /var/lib/durwinam ]; } && found+=("Durwinam panel")
...
function update_bot() {
    ...
    print_header "Updating Durwinam Bot"
...
EOF
    ...
    rm -rf "$TEMP_DIR"
    echo -e "\n\e[92mDurwinam Bot updated to latest version successfully!\033[0m"
...
function remove_bot() {
    echo -e "\e[33mStarting Durwinam Bot removal process...\033[0m"
    LOG_FILE="/var/log/remove_bot.log"
    echo "Log file: $LOG_FILE" > "$LOG_FILE"
    BOT_DIR="/var/www/html/mirzaprobotconfig"
    if [ ! -d "$BOT_DIR" ]; then
        echo -e "\e[31m[ERROR]\033[0m Durwinam Bot is not installed (/var/www/html/mirzaprobotconfig not found)." | tee -a "$LOG_FILE"
        echo -e "\e[33mNothing to remove. Exiting...\033[0m" | tee -a "$LOG_FILE"
        sleep 2
        exit 1
    fi
    read -p "Are you sure you want to remove Durwinam Bot and its dependencies? (y/n): " choice
    if [[ ! "$choice" =~ ^[Yy]$ ]]; then
        echo "Aborting..." | tee -a "$LOG_FILE"
        exit 0
    fi
    echo "Removing Durwinam Bot..." | tee -a "$LOG_FILE"
    ...
    echo -e "\e[92mDurwinam Bot, MySQL, and their dependencies have been completely removed.\033[0m" | tee -a "$LOG_FILE"
}
...
function migrate_to_pro() {
    ...
    if mysql -u "$ROOT_USER" -p"$ROOT_PASS" "$OLD_DB" -e "DESCRIBE durwinam_panel;" &>/dev/null; then
         mysql -u "$ROOT_USER" -p"$ROOT_PASS" "$OLD_DB" -e "UPDATE durwinam_panel SET status = 'active';"
    fi
