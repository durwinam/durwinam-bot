    ...
    if (count($channels_id) != 0) {
        ...
        if ($datain == "confirmchannel") {
            ...
            if ($partsaffiliates[0] == "affiliates") {
                ...
                $useraffiliates = select("user", "*", 'id', $affiliatesid, "select");
                if ($durwinamDiscountaffiliates['Discount'] == "onDiscountaffiliates") {
                    $durwinamDiscountaffiliates = select("affiliates", "*", null, null, "select");
                    $Balance_add_user = $useraffiliates['Balance'] + $durwinamDiscountaffiliates['price_Discount'];
                    update("user", "Balance", $Balance_add_user, "id", $affiliatesid);
                    $addbalancediscount = number_format($durwinamDiscountaffiliates['price_Discount'], 0);
...
if ($text == "/start" || $datain == "start" || $text == "start") {
    ...
    sendmessage($from_id, $textbotlang['textbot']['selectLocation'], $list_durwinam_panel_user, 'html');
    step('getdata', $from_id);
} elseif (preg_match('/locationnotuser_(.*)/', $datain, $dataget)) {
    $durwinam_list_get = select("durwinam_panel", "*", "code_panel", $dataget[1]);
    update("user", "Processing_value_four", $durwinam_list_get['code_panel'], "id", $from_id);
    $DataUserOut = $ManagePanel->DataUser($durwinam_list_get['name_panel'], $user['Processing_value']);
    ...
    if ($durwinamstatusextra == "onextra") {
    ...
    $durwinam = select("durwinam_panel", "*", "name_panel", $nameloc['Service_location'], "select");
    if ($durwinam['name_panel'] != null) {
        update("user", "Processing_value_four", $durwinam['name_panel'], "id", $from_id);
    }
    ...
    if (!in_array($status, ["active", "on_hold", "disabled", "Unknown"])) {
        ...
        if ($statustimeextra == "offtimeextraa")
            unset($keyboardsetting['inline_keyboard'][1][1]);
        if ($durwinamstatusextra == "offextra")
        ...
        $durwinamcount = select("durwinam_panel", "*", "status", "active", "count");
        ...
        if ($durwinam['type'] == "WGDashboard") {
            unset($keyboarddate['config']);
            unset($keyboarddate['changestatus']);
            unset($keyboarddate['change-location']);
            unset($keyboarddate['changelink']);
        }
        if ($durwinam['status_extend'] == "off_extend") {
            unset($keyboarddate['Extra_time']);
            unset($keyboarddate['Extra_volume']);
            unset($keyboarddate['extend']);
        }
        ...
        if ($durwinamcount == 1)
            unset($keyboarddate['change-location']);
        if ($durwinam['changeloc'] == "offchangeloc")
            unset($keyboarddate['change-location']);
        if ($statustimeextra == "offtimeextraa")
            unset($keyboarddate['Extra_time']);
        if ($durwinamstatusextra == "offextra")
        ...
        } elseif ($durwinam['type'] == "WGDashboard") {
            $textconnect = "";
        } else {
    ...
    if ($durwinam_list_get['type'] == "WGDashboard") {
        ...
        $subscriptionurl = $DataUserOut['subscription_url'];
        $urlimage = qrTempPath("{$durwinam_list_get['inboundid']}_{$nameloc['username']}.conf");
...
<code>$subscriptionurl</code>";
    ...
    $durwinam_list_get = select("durwinam_panel", "*", "name_panel", $nameloc['Service_location'], "select");
    if ($durwinam_list_get['status_extend'] == "off_extend") {
    ...
    $eextraprice = json_decode($durwinam_list_get['pricecustomvolume'], true);
    $custompricevalue = $eextraprice[$user['agent']];
    $mainvolume = json_decode($durwinam_list_get['mainvolume'], true);
    $mainvolume = $mainvolume[$user['agent']];
    $maxvolume = json_decode($durwinam_list_get['maxvolume'], true);
    ...
    $stmt->execute([
        ':service_location' => $durwinam_list_get['name_panel'],
        ':agent' => $user['agent'],
    ]);
    ...
    $maxvolume = json_decode($durwinam_list_get['maxvolume'], true);
    $maxvolume = $maxvolume[$user['agent']];
    $maintime = json_decode($durwinam_list_get['maintime'], true);
    $maintime = $maintime[$user['agent']];
    $maxtime = json_decode($durwinam_list_get['maxtime'], true);
    ...
    $eextraprice = json_decode($durwinam_list_get['pricecustomtime'], true);
    ...
    $stmt->bindParam(':code_panel', $durwinam_list_get['code_panel'], PDO::PARAM_STR);
    ...
    if ($nameloc['name_product'] == $textbotlang['users']['customSellVolume']['btnVolume'] || $nameloc['name_product'] == $textbotlang['users']['customSellVolume']['btnService']) {
        ...
        $stmt->bindParam(':code_product', $userdate['code_product'], PDO::PARAM_STR);
        $stmt->bindParam(':Location', $durwinam_list_get['name_panel'], PDO::PARAM_STR);
        $stmt->execute();
    ...
    if ($nameloc['name_product'] == $textbotlang['users']['customSellVolume']['btnVolume'] || $nameloc['name_product'] == $textbotlang['users']['customSellVolume']['btnService']) {
        ...
        $prodcut['inbounds'] = $durwinam_list_get['inboundid'];
    } else {
        $stmt = $pdo->prepare("SELECT * FROM product WHERE (Location = :service_location OR Location = '/all') AND agent = :agent AND code_product = :code_product");
        $stmt->execute([
            ':service_location' => $nameloc['Service_location'],
    ...
    if ($user['Balance'] < $pricelastextend && $user['agent'] != "n2" && intval($pricelastextend) != 0) {
        ...
        if ($durwinamdirectpay == "offdirectbuy") {
    ...
    if ($extend['status'] == false) {
        ...
        $textreports = sprintf($textbotlang['Admin']['reportgroup']['errorRenewService'], $durwinam_list_get['name_panel'], $nameloc['username'], $extend['msg']);
    ...
    if ($durwinam_list_get['sublink'] == "onsublink") {
    ...
    if ($durwinam_list_get['config'] == "onconfig") {
    ...
    $timejalali = jdate('Y/m/d H:i:s');
    $text_report = sprintf($textbotlang['Admin']['reportgroup']['linkChanged'], $from_id, $username, $nameloc['username'], $first_name, $durwinam_list_get['name_panel'], $user['agent'], $timejalali);
    if (strlen($setting['Channel_Report']) > 0) {
        telegram('sendmessage', [
            'chat_id' => $setting['Channel_Report'],
            'message_thread_id' => $otherservice,
    ...
    $eextraprice = json_decode($durwinam_list_get['priceextravolume'], true);
    ...
    $extra_volume = $ManagePanel->extra_volume($nameloc['username'], $durwinam_list_get['code_panel'], $data_limit);
    if ($extra_volume['status'] == false) {
        $extra_volume['msg'] = json_encode($extra_volume['msg']);
        $textreports = sprintf($textbotlang['Admin']['reportgroup']['errorExtraVolume'], $durwinam_list_get['name_panel'], $nameloc['username'], $extra_volume['msg']);
    ...
    update("user", "Processing_value", $nameloc['id_invoice'], "id", $from_id);
    $durwinam_list_get = select("durwinam_panel", "*", "name_panel", $nameloc['Service_location'], "select");
    if ($durwinam_list_get['changeloc'] == "offchangeloc") {
    ...
    Editmessagetext($from_id, $message_id, $textbotlang['textbot']['selectLocation'], $list_durwinam_panel_userschange);
    ...
    if ($userlimitlastfree < 0)
        $userlimitlastfree = 0;
    $Pricechange = select("durwinam_panel", "*", "code_panel", $dataget[1], "select")['priceChangeloc'];
    ...
    $nameloc = select("invoice", "*", "id_invoice", $id_invoice, "select");
    $durwinam_list_get = select("durwinam_panel", "*", "name_panel", $nameloc['Service_location'], "select");
    $durwinam_list_get_new = select("durwinam_panel", "*", "code_panel", $user['Processing_value_one'], "select");
    $limitchangeloc = json_decode($setting['limitnumber'], true);
    $limitfree = true;
    ...
    if ($durwinam_list_get_new['changeloc'] == "offchangeloc") {
        sendmessage($from_id, $textbotlang['users']['featureUnavailable2'], null, 'html');
        return;
    }
    if ($durwinam_list_get_new == false) {
        sendmessage($from_id, $textbotlang['users']['genericRestart'], null, 'html');
        return;
    }
    $Pricechange = $durwinam_list_get_new['priceChangeloc'];
    if ($nameloc['name_product'] == $textbotlang['users']['customSellVolume']['btnVolume'] || $nameloc['name_product'] == $textbotlang['users']['customSellVolume']['btnService']) {
    ...
    if ($product['inbounds'] != null) {
        $durwinam_list_get_new['inboundid'] = $prodcut['inbounds'];
    }
    if ($durwinam_list_get_new['type'] == "Manualsale" && $durwinam_list_get['url_panel'] == $durwinam_list_get_new['url_panel']) {
    ...
    $value = json_encode(array(
        "old_panel" => $durwinam_list_get['code_panel'],
        "new_panel" => $durwinam_list_get_new['code_panel'],
    ...
    $output = $DataUserOut['data_limit'] - $DataUserOut['used_traffic'];
    $RemainingVolume = $DataUserOut['data_limit'] ? formatBytes($output) : $textbotlang['common']['labels']['unlimitedShort'];
    if ($durwinam_list_get['url_panel'] == $durwinam_list_get_new['url_panel']) {
        $remove = $ManagePanel->RemoveUser($nameloc['Service_location'], $nameloc['username']);
        $dataoutput = $ManagePanel->createUser($durwinam_list_get_new['name_panel'], "usertest", $DataUserOut['username'], $datac);
        if ($dataoutput['username'] == null) {
            $dataoutput['msg'] = json_encode($dataoutput['msg']);
            sendmessage($from_id, $textbotlang['users']['sell']['errorConfig'], $keyboard, 'HTML');
            $texterros = sprintf($textbotlang['Admin']['reportgroup']['errorChangeLocation'], $dataoutput['msg'], $from_id, $username, $durwinam_list_get['name_panel'], $durwinam_list_get_new['name_panel']);
            if (strlen($setting['Channel_Report']) > 0) {
                telegram('sendmessage', [
                    'chat_id' => $setting['Channel_Report'],
                    'message_thread_id' => $errorreport,
    ...
    $output_config_link = "";
    if ($durwinam_list_get_new['sublink'] == "onsublink") {
        $output_config_link = $dataoutput['subscription_url'];
    }
    if ($durwinam_list_get_new['config'] == "onconfig") {
    ...
    $textchangeloc = sprintf($textbotlang['users']['changeLocation']['success'], $durwinam_list_get_new['name_panel'], $nameloc['username'], $RemainingVolume, $expirationDate, $day, $output_config_link);
    if (intval($Pricechange) != 0) {
        $Balance_Low_user = $user['Balance'] - $Pricechange;
        update("user", "Balance", $Balance_Low_user, "id", $from_id);
    }
    update("invoice", "Service_location", $durwinam_list_get_new['name_panel'], "username", $nameloc['username']);
    if ($durwinam_list_get_new['inboundid'] != null) {
        update("invoice", "inboundid", $durwinam_list_get_new['inboundid'], "username", $nameloc['username']);
    }
    Editmessagetext($from_id, $message_id, $textchangeloc, $keyboardextend);
    $balanceformatsell = number_format(select("user", "Balance", "id", $from_id, "select")['Balance'], 0);
    $format_byte = formatBytes($data_limit);
    $textreport = sprintf($textbotlang['Admin']['reportgroup']['locationChanged'], $from_id, $username, $durwinam_list_get['name_panel'], $durwinam_list_get_new['name_panel'], $nameloc['username'], $format_byte, $balanceformatsell);
    if (strlen($setting['Channel_Report']) > 0) {
        telegram('sendmessage', [
            'chat_id' => $setting['Channel_Report'],
            'message_thread_id' => $otherservice,
    ...
    $eextraprice = json_decode($durwinam_list_get['priceextratime'], true);
    ...
    $extra_time = $ManagePanel->extra_time($nameloc['username'], $durwinam_list_get['code_panel'], $extratimeday);
    if ($extra_time['status'] == false) {
        $extra_time['msg'] = json_encode($extra_time['msg']);
        $textreports = sprintf($textbotlang['Admin']['reportgroup']['errorExtraTime'], $durwinam_list_get['name_panel'], $nameloc['username'], $extra_time['msg']);
        sendmessage($from_id, $textbotlang['users']['extraVolume']['serviceError'], null, 'HTML');
        if (strlen($setting['Channel_Report']) > 0) {
            telegram('sendmessage', [
                'chat_id' => $setting['Channel_Report'],
    ...
    $locationproduct = select("durwinam_panel", "*", "TestAccount", "ONTestAccount", "count");
    ...
    if ($locationproduct != 1) {
        ...
        sendmessage($from_id, $textbotlang['textbot']['selectLocation'], $list_durwinam_usertest, 'html');
    }
}
if ($user['step'] == "createusertest" || preg_match('/locationtest_(.*)/', $datain, $dataget) || ($text == $textbotlang['textbot']['userTest'] || $datain == "usertestbtn" || $text == "usertest")) {
    ...
    $locationproduct = select("durwinam_panel", "*", "TestAccount", "ONTestAccount", "count");
    if ($locationproduct == 1) {
        $panel = select("durwinam_panel", "*", "TestAccount", "ONTestAccount", "select");
    ...
    $durwinam_list_get = select("durwinam_panel", "*", "code_panel", $location, "select");
    if ($durwinam_list_get['MethodUsername'] == $textbotlang['users']['customusername'] || $durwinam_list_get['MethodUsername'] == $textbotlang['keyboard']['customUsernameRandom']) {
        if ($user['step'] != "createusertest") {
    ...
    $durwinam_list_get = select("durwinam_panel", "*", "code_panel", $name_panel, "select");
    $text = strtolower($text);
    $username_ac = generateUsername($from_id, $durwinam_list_get['MethodUsername'], $user['username'], $randomString, $text, $durwinam_list_get['namecustom'], $user['namecustom']);
    $username_ac = strtolower($username_ac);
    $DataUserOut = $ManagePanel->DataUser($durwinam_list_get['name_panel'], $username_ac);
    $random_number = rand(1000000, 9999999);
    if (isset($DataUserOut['username']) || rowExists("invoice", "username", $username_ac)) {
        $username_ac = $random_number . "_" . $username_ac;
    }
    $datac = array(
        'expire' => strtotime(date("Y-m-d H:i:s", strtotime("+" . $durwinam_list_get['time_usertest'] . "hours"))),
        'data_limit' => $durwinam_list_get['val_usertest'] * 1048576,
        'from_id' => $from_id,
        'username' => $username,
        'type' => 'usertest'
    );
    ...
    $info_product['price_product'] = "0";
    $Status = "active";
    $stmt->execute([$from_id, $randomString, $username_ac, $date, $durwinam_list_get['name_panel'], $info_product['name_product'], $info_product['price_product'], $durwinam_list_get['val_usertest'], $durwinam_list_get['time_usertest'], $Status, $notifctions]);
    $dataoutput = $ManagePanel->createUser($durwinam_list_get['name_panel'], "usertest", $username_ac, $datac);
    if ($dataoutput['username'] == null) {
        $dataoutput['msg'] = json_encode($dataoutput['msg']);
        sendmessage($from_id, $textbotlang['users']['usertest']['errorcreat'], $keyboard, 'html');
        $texterros = sprintf($textbotlang['Admin']['reportgroup']['errorTestAccountCreate'], $dataoutput['msg'], $from_id, $username, $durwinam_list_get['name_panel']);
        if (strlen($setting['Channel_Report']) > 0) {
            telegram('sendmessage', [
                'chat_id' => $setting['Channel_Report'],
                'message_thread_id' => $errorreport,
    ...
    $output_config_link = "";
    $config = "";
    $output_config_link = $durwinam_list_get['sublink'] == "onsublink" ? $dataoutput['subscription_url'] : "";
    ...
    $textbotlang['textbot']['afterText'] = $durwinam_list_get['type'] == "ibsng" || $durwinam_list_get['type'] == "mikrotik" ? $textbotlang['textbot']['afterPayIbsng'] : $textbotlang['textbot']['afterText'];
    ...
    $textcreatuser = str_replace('{day}', $durwinam_list_get['time_usertest'], $textcreatuser);
    $textcreatuser = str_replace('{volume}', $durwinam_list_get['val_usertest'], $textcreatuser);
    $textcreatuser = str_replace('{config}', "<code>{$output_config_link}</code>", $textcreatuser);
    $textcreatuser = str_replace('{links}', $config, $textcreatuser);
    $textcreatuser = str_replace('{links2}', $output_config_link, $textcreatuser);
    if ($durwinam_list_get['type'] == "ibsng" || $durwinam_list_get['type'] == "mikrotik") {
        $textcreatuser = str_replace('{password}', $dataoutput['subscription_url'], $textcreatuser);
        update("invoice", "user_info", $dataoutput['subscription_url'], "id_invoice", $randomString);
    }
    sendMessageService($durwinam_list_get, $dataoutput['configs'], $output_config_link, $dataoutput['username'], $usertestinfo, $textcreatuser, $randomString);
    sendmessage($from_id, $textbotlang['users']['selectoption'], $keyboard, 'HTML');
    step('home', $from_id);
    if ($durwinam_list_get['MethodUsername'] == $textbotlang['keyboard']['customTextSequential'] || $durwinam_list_get['MethodUsername'] == $textbotlang['keyboard']['usernameSequential'] || $durwinam_list_get['MethodUsername'] == $textbotlang['keyboard']['numericIdSequential'] || $durwinam_list_get['MethodUsername'] == $textbotlang['keyboard']['agentCustomTextSequential']) {
        $value = intval($user['number_username']) + 1;
        update("user", "number_username", $value, "id", $from_id);
        if ($durwinam_list_get['MethodUsername'] == $textbotlang['keyboard']['customTextSequential'] || $durwinam_list_get['MethodUsername'] == $textbotlang['keyboard']['agentCustomTextSequential']) {
    ...
    $timejalali = jdate('Y/m/d H:i:s');
    $text_report = sprintf($textbotlang['Admin']['reportgroup']['testAccountCreated'], $from_id, $username, $username_ac, $first_name, $durwinam_list_get['name_panel'], $durwinam_list_get['time_usertest'], $durwinam_list_get['val_usertest'], $randomString, $user['agent'], $user['number'], $timejalali);
    if (strlen($setting['Channel_Report']) > 0) {
        telegram('sendmessage', [
            'chat_id' => $setting['Channel_Report'],
    ...
    $locationproduct = $pdo->prepare("SELECT * FROM durwinam_panel  WHERE status = 'active' AND (agent = ? OR agent = 'all')");
    ...
    if (($locationproduct)->rowCount() == 1) {
        ...
        $locationproduct = select("durwinam_panel", "*", "name_panel", $location, "select");
        ...
        if ($setting['statuscategory'] == "offcategory") {
            $durwinam_list_get = $locationproduct;
            $eextraprice = json_decode($durwinam_list_get['pricecustomvolume'], true);
            $custompricevalue = $eextraprice[$user['agent']];
            $mainvolume = json_decode($durwinam_list_get['mainvolume'], true);
            $mainvolume = $mainvolume[$user['agent']];
            ...
            if ($setting['statuscategorygenral'] == "oncategorys") {
                $durwinam_list_get = select("durwinam_panel", "*", "name_panel", $location, "select");
                if ($setting['statusnamecustom'] == 'onnamecustom') {
                    $backuser = "buyback";
                } else {
                    $backuser = "backuser";
                ...
                $durwinam_list_get = select("durwinam_panel", "*", "name_panel", $location, "select");
                $statuscustomvolume = json_decode($durwinam_list_get['customvolume'], true)[$user['agent']];
                if ($durwinam_list_get['MethodUsername'] == $textbotlang['users']['customusername'] || $durwinam_list_get['MethodUsername'] == $textbotlang['keyboard']['customUsernameRandom']) {
                    $datakeyboard = "prodcutservices_";
                } else {
                    $datakeyboard = "prodcutservice_";
                }
                if ($statuscustomvolume == "1" && $durwinam_list_get['type'] != "Manualsale") {
                    $statuscustom = true;
                } else {
                    $statuscustom = false;
                }
                ...
                if ($datain == "buy") {
                    Editmessagetext($from_id, $message_id, $textproduct, KeyboardProduct($durwinam_list_get['name_panel'], $query, $user['pricediscount'], $datakeyboard, $statuscustom));
                } else {
                    sendmessage($from_id, $textproduct, KeyboardProduct($durwinam_list_get['name_panel'], $query, $user['pricediscount'], $datakeyboard, $statuscustom), 'HTML');
                }
            }
        } else {
            $nullproduct = select("product", "*", null, null, "count");
            ...
            $monthkeyboard = keyboardTimeCategory($durwinam_list_get['name_panel'], $user['agent'], "productmonth_", $back, $statuscustom, false);
    ...
    if ($datain == "buy" || $datain == "buybacktow" || $datain == "buyback") {
        Editmessagetext($from_id, $message_id, $textbotlang['textbot']['selectLocation'], $list_durwinam_panel_user);
    } else {
        sendmessage($from_id, $textbotlang['textbot']['selectLocation'], $list_durwinam_panel_user, 'HTML');
    }
    ...
    if ($datain != "backproduct") {
        $location = select("durwinam_panel", "*", "code_panel", $dataget[1], "select")['name_panel'];
    } else {
        $location = $userdate['name_panel'];
    }
    $durwinam_list_get = select("durwinam_panel", "*", "name_panel", $location, "select");
    $locationproductcount = select("durwinam_panel", "*", "name_panel", $location, "count");
    $stmt = $pdo->prepare("SELECT * FROM invoice WHERE (status = 'active' OR status = 'end_of_time' OR status = 'end_of_volume' OR status = 'sendedwarn' OR Status = 'send_on_hold') AND  Service_location = :mp2");
    $stmt->execute([':mp2' => $durwinam_list_get['name_panel']]);
    $countinovoice = $stmt->rowCount();
    if (is_numeric($durwinam_list_get['limit_panel'])) {
        if ($countinovoice >= intval($durwinam_list_get['limit_panel'])) {
    ...
    if ($setting['statuscategory'] == "offcategory") {
        if ($setting['statuscategorygenral'] == "oncategorys") {
            ...
            Editmessagetext($from_id, $message_id, $textbotlang['users']['sell']['serviceSelect'], KeyboardProduct($durwinam_list_get['name_panel'], $query, $user['pricediscount'], $datakeyboard, $statuscustom, $back));
        }
    } else {
        $nullproduct = select("product", "*", null, null, "count");
        if ($nullproduct == 0) {
        ...
        if ($statuscustomvolume == "1" && $durwinam_list_get['type'] != "Manualsale")
            $statuscustom = true;
        $monthkeyboard = keyboardTimeCategory($durwinam_list_get['name_panel'], $user['agent'], "productmonth_", "buybacktow", $statuscustom, false);
        Editmessagetext($from_id, $message_id, $textbotlang['users']['sell']['selectDuration'], $monthkeyboard);
    }
    ...
    $durwinam_list_get = select("durwinam_panel", "*", "name_panel", $userdate['name_panel'], "select");
    $statuscustomvolume = json_decode($durwinam_list_get['customvolume'], true)[$user['agent']];
    if ($durwinam_list_get['MethodUsername'] == $textbotlang['users']['customusername'] || $durwinam_list_get['MethodUsername'] == $textbotlang['keyboard']['customUsernameRandom']) {
        $datakeyboard = "prodcutservices_";
    } else {
    ...
    Editmessagetext($from_id, $message_id, $textbotlang['users']['sell']['serviceSelectFirst'], KeyboardProduct($durwinam_list_get['name_panel'], $query, $user['pricediscount'], $datakeyboard, $statuscustom));
} elseif (preg_match('/^productmonth_(\w+)/', $datain, $dataget)) {
    $monthenumber = $dataget[1];
    $userdate = json_decode($user['Processing_value'], true);
    $durwinam_list_get = select("durwinam_panel", "*", "name_panel", $userdate['name_panel']);
    if ($setting['statuscategorygenral'] == "oncategorys") {
        savedata("save", "monthproduct", $monthenumber);
        $durwinam_list_get = select("durwinam_panel", "*", "name_panel", $userdate['name_panel'], "select");
        $stmt = $pdo->prepare("SELECT * FROM durwinam_panel  WHERE status = 'active' AND (agent = :mp3 OR agent = 'all')");
        $stmt->execute([':mp3' => $user['agent']]);
        $count_panel = $stmt->rowCount();
        if ($count_panel == 1) {
            $back = "buybacktow";
        } else {
            $back = "location_{$durwinam_list_get['code_panel']}";
        }
        Editmessagetext($from_id, $message_id, $textbotlang['users']['sell']['selectCategory'], KeyboardCategory($durwinam_list_get['name_panel'], $user['agent'], $back));
    } else {
    ...
    $durwinam_list_get = select("durwinam_panel", "*", "name_panel", $userdate['name_panel'], "select");
    if ($durwinam_list_get['status'] == "disable") {
    ...
    $randomString = bin2hex(random_bytes(2));
    $text = strtolower($text);
    $username_ac = generateUsername($from_id, $durwinam_list_get['MethodUsername'], $username, $randomString, $text, $durwinam_list_get['namecustom'], $user['namecustom']);
    $username_ac = strtolower($username_ac);
    $DataUserOut = $ManagePanel->DataUser($durwinam_list_get['name_panel'], $username_ac);
    $random_number = rand(1000000, 9999999);
    if (isset($DataUserOut['username']) || rowExists("invoice", "username", $username_ac)) {
    ...
    if ($user['step'] != "getvolumecustomuser" && !in_array($durwinam_list_get['MethodUsername'], [$textbotlang['common']['labels']['customUsername'], $textbotlang['common']['labels']['customUsernameRandom']])) {
    ...
    $Status = "unpaid";
    $stmt->execute([$from_id, $randomString, $username_ac, $date, $durwinam_list_get['name_panel'], $info_product['name_product'], $priceproduct, $info_product['Volume_constraint'], $info_product['Service_time'], $Status, $userdate['nameconfig'] ?? null, $user['affiliates'], $notifctions]);
    ...
    $dataoutput = $ManagePanel->createUser($durwinam_list_get['name_panel'], $info_product['code_product'], $username_ac, $datac);
    if (!isset($dataoutput['username']) || $dataoutput['username'] === null || $dataoutput['username'] === '') {
        ...
        sendmessage($from_id, $textbotlang['users']['sell']['errorConfig'], $keyboard, 'HTML');
        $texterros = sprintf($textbotlang['Admin']['reportgroup']['errorSubscriptionCreate'], $dataoutput['msg'], $from_id, $username, $durwinam_list_get['name_panel']);
        if (strlen($setting['Channel_Report']) > 0) {
            telegram('sendmessage', [
                'chat_id' => $setting['Channel_Report'],
                'message_thread_id' => $errorreport,
    ...
    sendMessageService($durwinam_list_get, $dataoutput['configs'], $output_config_link, $dataoutput['username'], $Shoppinginfo, $textcreatuser, $randomString);
    sendmessage($from_id, $textbotlang['users']['selectoption'], $keyboard, 'HTML');
    ...
    if ($affiliatescommission['status_commission'] == "oncommission" && ($user['affiliates'] != null && intval($user['affiliates']) != 0)) {
        if ($durwinamporsant_one_buy['porsant_one_buy'] == "on_buy_porsant") {
            if ($countinvoice == 1) {
    ...
    $locationproduct = $pdo->prepare("SELECT * FROM durwinam_panel");
    $locationproduct->execute();
    if (($locationproduct)->rowCount() == 0) {
        sendmessage($from_id, $textbotlang['users']['sell']['nullPanel'], null, 'HTML');
        return;
    ...
    sendmessage($from_id, $textbotlang['textbot']['selectLocation'], $list_durwinam_panel_userom, 'HTML');
    update("user", "Processing_value_four", $text, "id", $from_id);
    step('home', $from_id);
} elseif (preg_match('/^locationom_(.*)/', $datain, $dataget)) {
    $location = select("durwinam_panel", "*", "code_panel", $dataget[1], "select")['name_panel'];
    $durwinam_list_get = select("durwinam_panel", "*", "code_panel", $dataget[1], "select");
    $nullproduct = select("product", "*", null, null, "count");
    if ($nullproduct == 0) {
        sendmessage($from_id, $textbotlang['users']['sell']['nullProduct'], null, 'HTML');
        return;
    ...
    $query = "SELECT * FROM product WHERE (Location = '$location' OR Location = '/all')AND agent= '{$user['agent']}'";
    Editmessagetext($from_id, $message_id, $textbotlang['users']['sell']['serviceSelect'], KeyboardProduct($durwinam_list_get['name_panel'], $query, $user['pricediscount'], $datakeyboard, $statuscustom, "backuser", null, "customsellvolumeom"));
} elseif ($datain == "customsellvolumeom") {
    $durwinam_list_get = select("durwinam_panel", "*", "name_panel", $user['Processing_value'], "select");
    $eextraprice = json_decode($durwinam_list_get['pricecustomvolume'], true);
    $custompricevalue = $eextraprice[$user['agent']];
    ...
    if ($info_product['inbounds'] != null) {
        $durwinam_list_get['inboundid'] = $info_product['inbounds'];
    }
    $notifctions = json_encode(array(
        'volume' => false,
        'time' => false,
    ...
    for ($i = 0; $i < $user['Processing_value_four']; $i++) {
        ...
        $get_username_Check = $ManagePanel->DataUser($durwinam_list_get['name_panel'], $username_acc);
        ...
        $dataoutput = $ManagePanel->createUser($durwinam_list_get['name_panel'], $info_product['code_product'], $username_acc, $datac);
        if ($dataoutput['username'] == null) {
            $dataoutput['msg'] = json_encode($dataoutput['msg']);
            sendmessage($from_id, $textbotlang['users']['sell']['errorConfig'], $keyboard, 'HTML');
            $texterros = sprintf($textbotlang['Admin']['reportgroup']['errorBulkAccountCreate'], $dataoutput['msg'], $from_id, $username, $durwinam_list_get['name_panel']);
            if (strlen($setting['Channel_Report']) > 0) {
                telegram('sendmessage', [
                    'chat_id' => $setting['Channel_Report'],
                    'message_thread_id' => $errorreport,
...
if (preg_match('/^sendresidcart-(.*)/', $datain, $dataget)) {
    ...
    $locations = select("durwinam_panel", "*", "code_panel", $dataget[2], "select");
    ...
    $durwinam_list_get = select("durwinam_panel", "*", "name_panel", $user['Processing_value_one'], "select");
    $eextraprice = json_decode($durwinam_list_get['priceextravolume'], true);
    $extrapricevalue = $eextraprice[$user['agent']];
    $priceextra = $extrapricevalue * $text;
    $keyboardsetting = json_encode([
    ...
    $extra_volume = $ManagePanel->extra_volume($user['Processing_value'], $durwinam_list_get['code_panel'], $data_limit_new);
    if ($extra_volume['status'] == false) {
        $extra_volume['msg'] = json_encode($extra_volume['msg']);
...
if (isset($update['message']['successful_payment'])) {
    ...
    $username = $dataget[1];
    $location = select("durwinam_panel", "*", "code_panel", $user['Processing_value_four'], "select");
    if ($location == false) {
    ...
    $query = "SELECT * FROM product WHERE (Location = '$location' OR Location = '/all') AND agent= '{$user['agent']}'";
    $durwinam_list_get = select("durwinam_panel", "*", "code_panel", $location, "select");
    if (empty($durwinam_list_get)) {
        sendmessage($from_id, $textbotlang['users']['status']['infoUnavailable'], null, 'HTML');
        step('home', $from_id);
        return;
    }
    $statuscustomvolume = json_decode($durwinam_list_get['customvolume'] ?? '[]', true)[$user['agent']] ?? null;
    if ($durwinam_list_get['MethodUsername'] == $textbotlang['users']['customusername'] || $durwinam_list_get['MethodUsername'] == $textbotlang['keyboard']['customUsernameRandom']) {
        $datakeyboard = "prodcutservicesom_";
    } else {
        $datakeyboard = "prodcutserviceom_";
    ...
    Editmessagetext($from_id, $message_id, $textbotlang['users']['extend']['selectservice'], KeyboardProduct($durwinam_list_get['name_panel'], $query, $user['pricediscount'], "serviceextendselects-", false, "backuser", $username));
    ...
    $prodcut = $stmt->fetch(PDO::FETCH_ASSOC);
    $durwinam_list_get = select("durwinam_panel", "*", "name_panel", $user['Processing_value'], "select");
    $DataUserOut = $ManagePanel->DataUser($durwinam_list_get['name_panel'], $usernamePanelExtends);
    if ($DataUserOut['status'] == "Unsuccessful") {
    ...
    update("user", "Balance", $Balance_Low_user, "id", $from_id);
    $extend = $ManagePanel->extend($durwinam_list_get['Methodextend'], $prodcut['Volume_constraint'], $prodcut['Service_time'], $usernamePanelExtends, $prodcut['code_product'], $durwinam_list_get['code_panel']);
    if ($extend['status'] == false) {
        $extend['msg'] = json_encode($extend['msg']);
        $textreports = sprintf($textbotlang['Admin']['reportgroup']['errorRenewService2'], $durwinam_list_get['name_panel'], $usernamePanelExtends, $extend['msg']);
        sendmessage($from_id, $textbotlang['users']['extend']['errorSupport'], null, 'HTML');
        if (strlen($setting['Channel_Report']) > 0) {
            telegram('sendmessage', [
                'chat_id' => $setting['Channel_Report'],
    ...
    $timejalali = jdate('Y/m/d H:i:s');
    $text_report = sprintf($textbotlang['Admin']['reportgroup']['renewalDetails'], $from_id, $username, $usernamePanelExtends, $first_name, $durwinam_list_get['name_panel'], $prodcut['name_product'], $prodcut['Volume_constraint'], $prodcut['Service_time'], $prodcut['price_product'], $balanceformatsell, $timejalali);
    if (strlen($setting['Channel_Report']) > 0) {
        telegram('sendmessage', [
            'chat_id' => $setting['Channel_Report'],
            'message_thread_id' => $otherservice,
