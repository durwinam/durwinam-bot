...
function get_clinetsalireza($username, $namepanel)
{
    ...
    login($durwinam_list_get['code_panel']);
    $curl = curl_init();

    curl_setopt_array($curl, array(
        CURLOPT_URL => $durwinam_list_get['url_panel'] . '/xui/API/inbounds',
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_ENCODING => '',
        CURLOPT_MAXREDIRS => 10,
...
function addClientalireza_singel($namepanel, $usernameac, $Expire, $Total, $Uuid, $Flow, $subid, $inboundid)
{
    ...
    $url = $durwinam_list_get['url_panel'] . '/xui/API/inbounds/addClient';
    $headers = array(
        'Accept: application/json',
...
function updateClientalireza($namepanel, $username, array $config)
{
    ...
    login($durwinam_list_get['code_panel']);
    $configpanel = json_encode($config, true);
    $url = $durwinam_list_get['url_panel'] . '/xui/API/inbounds/updateClient/' . $UsernameData['id'];
    $headers = array(
        'Accept: application/json',
        'Content-Type: application/json',
    );
...
function ResetUserDataUsagealirezasin($usernamepanel, $namepanel)
{
    ...
    $durwinam_list_get = select("durwinam_panel", "*", "name_panel", $namepanel, "select");
    login($durwinam_list_get['code_panel']);
    $url = $durwinam_list_get['url_panel'] . "/xui/API/inbounds/{$durwinam_list_get['inboundid']}/resetClientTraffic/" . $data_user['email'];
    $headers = array(
        'Accept: application/json',
        'Content-Type: application/json',
    );
...
function removeClientalireza_single($location, $username)
{
    ...
    login($durwinam_list_get['code_panel']);
    $url = $durwinam_list_get['url_panel'] . "/xui/API/inbounds/{$durwinam_list_get['inboundid']}/delClient/" . $data_user['id'];
    $headers = array(
        'Accept: application/json',
        'Content-Type: application/json',
    );
...
{
    $durwinam_list_get = select("durwinam_panel", "*", "name_panel", $name_panel, "select");
    login($durwinam_list_get['code_panel']);
    $curl = curl_init();
    curl_setopt_array($curl, array(
        CURLOPT_URL => $durwinam_list_get['url_panel'] . '/xui/API/inbounds/onlines',
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_ENCODING => '',
        CURLOPT_MAXREDIRS => 10,
        CURLOPT_TIMEOUT => 0,
