...
//------------------  [ list panel ]----------------//
$namepanel = [];
    $allPanelRows = $pdo->query("SELECT * FROM durwinam_panel")->fetchAll(PDO::FETCH_ASSOC);
    ...
    $json_list_durwinam_panel = json_encode($list_durwinam_panel);
    //------------------  [ list panel inline ]----------------//
    $list_durwinam_panel_edit_product = ['inline_keyboard' => []];
    foreach ($allPanelRows as $row) {
        $list_durwinam_panel_edit_product['inline_keyboard'][] = [['text' => $row['name_panel'], 'callback_data' => 'locationedit_' . $row['code_panel']]];
    }
    $list_durwinam_panel_edit_product['inline_keyboard'][] = [['text' => $textbotlang['keyboard']['allPanels'], 'callback_data' => 'locationedit_all']];
    $list_durwinam_panel_edit_product['inline_keyboard'][] = [['text' => $textbotlang['keyboard']['backToPreviousMenu'], 'callback_data' => 'backproductadmin']];
    $list_durwinam_panel_edit_product = json_encode($list_durwinam_panel_edit_product);
//------------------  [ list channel ]----------------//
$list_channels = [];
...
//------------------  [ listpanelusers ]----------------//
$stmt = $pdo->prepare("SELECT * FROM durwinam_panel WHERE status = 'active' AND (agent = :agent OR agent = 'all')");
...
$list_durwinam_panel_users = ['inline_keyboard' => []];
$panelcount = select("durwinam_panel", "*", "status", "active", "count");
if ($panelcount > 10) {
    $temp_row = [];
    foreach ($activePanelRows as $result) {
        ...
        if (count($temp_row) == 2) {
            $list_durwinam_panel_users['inline_keyboard'][] = $temp_row;
            $temp_row = [];
        }
    }
    if (!empty($temp_row)) {
...
$list_durwinam_panel_user = json_encode($list_durwinam_panel_users);


//------------------  [ listpanelusers omdhe ]----------------//
$list_durwinam_panel_users_om = ['inline_keyboard' => []];
foreach ($activePanelRows as $result) {
    if ($result['hide_user'] != null and in_array($from_id, json_decode($result['hide_user'], true)))
        continue;
    $list_durwinam_panel_users_om['inline_keyboard'][] = [
...
$list_durwinam_panel_userom = json_encode($list_durwinam_panel_users_om);

//------------------  [ change location ]----------------//
$list_durwinam_panel_users_change = ['inline_keyboard' => []];
if ($panelcount > 10) {
    $temp_row = [];
    foreach ($activePanelRows as $result) {
        ...
        if (count($temp_row) == 2) {
            $list_durwinam_panel_users_change['inline_keyboard'][] = $temp_row;
            $temp_row = [];
        }
    }
    if (!empty($temp_row)) {
...
$list_durwinam_panel_userschange = json_encode($list_durwinam_panel_users_change);


//------------------  [ listpanelusers test ]----------------//
$stmt = $pdo->prepare("SELECT * FROM durwinam_panel WHERE TestAccount = 'ONTestAccount' AND (agent = :agent OR agent = 'all')");
$stmt->bindValue(':agent', $users['agent'], PDO::PARAM_STR);
$stmt->execute();
$list_durwinam_panel_usertest = ['inline_keyboard' => []];
...
$list_durwinam_usertest = json_encode($list_durwinam_panel_usertest);
...
$keyboardtypepanel = json_encode([
    'inline_keyboard' => [
        [
            ['text' => $textbotlang['keyboard']['durwinam'], 'callback_data' => "typepanel#durwinam"],
        ...
        [
            ...
            ['text' => $textbotlang['keyboard']['durwinamAgentPanel'], 'callback_data' => "typepanel#durwinam_agent"]
        ],
        [
...
$panelechekc = select("durwinam_panel", "*", "MethodUsername", $textbotlang['keyboard']['usernameMethodAgentCustom'], "count");
if ($setting['inlinebtnmain'] == "oninline") {
    $keyboardagent = [
        'inline_keyboard' => [
            [
