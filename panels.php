...
require_once __DIR__ . '/Durwinam.php';
...
require_once __DIR__ . '/durwinam_agent.php';
...
class ManagePanel
{
    ...
    function createUser($name_panel, $code_product, $usernameC, array $Data_Config)
    {
        ...
        $Get_Data_Panel = select("durwinam_panel", "*", "name_panel", $name_panel, "select");
        ...
        if ($Get_Data_Panel['type'] == "durwinam") {
            //create user
        ...
        } elseif ($Get_Data_Panel['type'] == "durwinam_agent") {
            //create user
            $ConnectToPanel = create_user_durwinam($Get_Data_Panel, $data_limit / pow(1024, 3), $expire == 0 ? 0 : ($expire - time()) / 86400, $usernameC);
    ...
    function DataUser($name_panel, $username)
    {
        ...
        if ($Get_Data_Panel['type'] == "durwinam") {
            ...
            $UsernameData = get_user_data_durwinam($Get_Data_Panel, $username);
            if (!empty($UsernameData['error'])) {
                $Output = array(
                    'status' => 'Unsuccessful',
                    'msg' => $UsernameData['error']
    ...
    function Revoke_sub($name_panel, $username)
    {
        ...
        if ($Get_Data_Panel['type'] == "durwinam") {
            ...
            $revoke_sub = revoke_service_durwinam($Get_Data_Panel, $username);
    ...
    function RemoveUser($name_panel, $username)
    {
        ...
        if ($Get_Data_Panel['type'] == "durwinam") {
            ...
            $UsernameData = remove_service_durwinam($Get_Data_Panel, $username);
            if (isset($UsernameData['error'])) {
                $Output = array(
                    'status' => 'Unsuccessful',
                    'msg' => $UsernameData['msg']
    ...
    {
        $panel = select("durwinam_panel", "*", "name_panel", $name_panel, "select");
        if ($panel == false) {
            return array(
                'status' => false,
                'msg' => 'data not found'
        ...
        } elseif ($panel['type'] == "durwinam_agent") {
            return array(
                'status' => true,
                'msg' => 'successful'
            );
    ...
    {
        global $textbotlang;
        $panel = select("durwinam_panel", "*", "code_panel", $name_panel, "select");
        ...
        if ($panel['type'] == "durwinam") {
            ...
            $extend = extend_service_durwinam($panel, $new_limit, $time_day, $username);
    ...
    function extra_volume($username_account, $code_panel, $limit_volume_new)
    {
        ...
        if ($panel['type'] == "durwinam") {
            ...
            $volume_add = add_volume_service_durwinam($panel, $limit_volume_new, $username_account);
    ...
    function extra_time($username_account, $code_panel, $limit_time_new)
    {
        ...
        if ($panel['type'] == "durwinam") {
            ...
            $time_add = add_time_service_durwinam($panel, $new_limit, $username_account);
