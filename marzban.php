...
function token_panel($code_panel, $verify = true)
{
    ...
    if (isset($body['access_token'])) {
        ...
        update("durwinam_panel", "datelogin", $data, 'name_panel', $panel['name_panel']);
    }
    return $body;
}
#-----------------------------#
...
function getuser($username_account, $location)
{
    ...
    $Check_token = token_panel($durwinam_list_get['code_panel']);
    if (!empty($Check_token['error'])) {
        return $Check_token;
    }
    $url = $durwinam_list_get['url_panel'] . '/api/user/' . $username_account;
    $headers = array(
        'accept: application/json'
    );
...
function Get_Nodes($location)
{
    ...
    $url = $durwinam_list_get['url_panel'] . '/api/nodes';
    $headers = array(
        'accept: application/json'
    );
    $req = new CurlRequest($url);
...
function Get_usage_Nodes($location)
{
    ...
    $url = $durwinam_list_get['url_panel'] . '/api/nodes/usage';
    $headers = array(
        'accept: application/json'
    );
    $req = new CurlRequest($url);
...
function Get_Node($location, $Nodeid)
{
    ...
    $url = $durwinam_list_get['url_panel'] . '/api/node/' . $Nodeid;
    $headers = array(
        'accept: application/json'
    );
    $req = new CurlRequest($url);
...
function getusers($location, $status)
{
    ...
    $url = $durwinam_list_get['url_panel'] . '/api/users?status=' . $status;
...
function getinbounds($location)
{
    $durwinam_list_get = select("durwinam_panel", "*", "name_panel", $location, "select");
    $Check_token = token_panel($durwinam_list_get['code_panel']);
    $url = $durwinam_list_get['url_panel'] . '/api/inbounds';
    $header_value = 'Bearer ';
...
function ResetUserDataUsage($username_account, $location)
{
    ...
    $url = $durwinam_list_get['url_panel'] . '/api/user/' . $username_account . '/reset';
...
function revoke_sub($username_account, $location)
{
    ...
    $url = $durwinam_list_get['url_panel'] . '/api/user/' . $username_account . '/revoke_sub';
    $headers = array(
        'accept: application/json'
    );
    $req = new CurlRequest($url);
...
function adduser($location, $data_limit, $username_ac, $timestamp, $note = '', $data_limit_reset = 'no_reset', $name_product = false)
{
    ...
    $url = $durwinam_list_get['url_panel'] . "/api/user";
    if ($durwinam_list_get['inbounds'] != null and $durwinam_list_get['inbounds'] != "null") {
        if ($name_product != false and $name_product != "usertest") {
            ...
            if ($product == false || $product['inbounds'] == false) {
                $inbounds = json_decode($durwinam_list_get['inbounds'], true);
            } else {
                $inbounds = json_decode($product['inbounds'], true);
                $durwinam_list_get['proxies'] = $product['proxies'];
            }
        } else {
            $inbounds = json_decode($durwinam_list_get['inbounds'], true);
        }
    ...
    if ($durwinam_list_get['version_panel'] == "1") {
        $data = array(
            "proxy_settings" => json_decode($durwinam_list_get['proxies']),
        ...
        if ($name_product == "usertest") {
            if ($durwinam_list_get['on_hold_test'] == "0") {
                if ($timestamp == 0) {
                    $data["expire"] = 0;
                } else {
            ...
            if ($durwinam_list_get['conecton'] == "offconecton") {
                if ($timestamp == 0) {
                    $data["expire"] = 0;
                } else {
                    $data["expire"] = date('c', $timestamp);
        ...
        $data = array(
            "proxies" => json_decode($durwinam_list_get['proxies']),
            "data_limit" => $data_limit,
            "username" => $username_ac,
            "note" => $note,
            "data_limit_reset_strategy" => $data_limit_reset
...
function Get_System_Stats($location)
{
    $durwinam_list_get = select("durwinam_panel", "*", "name_panel", $location, "select");
    $Check_token = token_panel($durwinam_list_get['code_panel']);
    $url = $durwinam_list_get['url_panel'] . '/api/system';
    $header_value = 'Bearer ';
...
function removeuser($location, $username)
{
    ...
    $url = $durwinam_list_get['url_panel'] . '/api/user/' . $username;
    $headers = array(
        'accept: application/json'
    );
    $req = new CurlRequest($url);
...
function Modifyuser_node($location, $id_node, array $data)
{
    $durwinam_list_get = select("durwinam_panel", "*", "name_panel", $location, "select");
    $Check_token = token_panel($durwinam_list_get['code_panel']);
    $url = $durwinam_list_get['url_panel'] . '/api/node/' . $id_node;
    $payload = json_encode($data);
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, $url);
...
function hosts($location)
{
    $durwinam_list_get = select("durwinam_panel", "*", "name_panel", $location, "select");
    $Check_token = token_panel($durwinam_list_get['code_panel']);
    $url = $durwinam_list_get['url_panel'] . '/api/hosts';
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, $url);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
...
function reconnect_node($location, $id_node)
{
    ...
    $url = $durwinam_list_get['url_panel'] . '/api/node/' . $id_node . '/reconnect';
    $headers = array(
        'accept: application/json'
    );
    $req = new CurlRequest($url);
...
function get_list_update($location, $username)
{
    ...
    $url = $durwinam_list_get['url_panel'] . "/api/user/$username/sub_update?offset=0&limit=1";
    $headers = array(
        'accept: application/json'
    );
    $req = new CurlRequest($url);
