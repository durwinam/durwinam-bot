...
if (in_array($text, $textadmin) || $datain == "admin") {
    ...
    if (in_array($user['step'], ["updatetime", "val_usertest", "getlimitnew", "GetusernameNew", "GeturlNew", "protocolset", "updatemethodusername", "GetNameNew", "getprotocol", "getprotocolremove", "GetpaawordNew", "updateextendmethod", "setpricechangelocation"])) {
        $typepanel = select("durwinam_panel", "*", "name_panel", $user['Processing_value'], "select");
    ...
    $sumpanel = select("durwinam_panel", "*", null, null, "count");
    ...
    if (rowExists("durwinam_panel", "name_panel", $text)) {
    ...
    if ($userdata['type'] == "hiddify") {
        ...
        savedata("save", "password", "null");
        return;
    } elseif ($userdata['type'] == "s_ui" || $userdata['type'] == "WGDashboard" || $userdata['type'] == "x-ui_single" || $userdata['type'] == "durwinam_agent" || $userdata['type'] == "rebecca") {
...
elseif ($datain == "systemsms") {
    ...
    if ($userdata['typeusermessage'] == "customer") {
        $stmt = $pdo->prepare("SELECT * FROM durwinam_panel WHERE agent = :agent OR agent = 'all'");
        ...
        if ($agent == "all") {
            if ($typeusermessage == "all") {
                ...
                if ($userdata['selectpanel'] == "all") {
                    ...
                    $panel = select("durwinam_panel", "*", "code_panel", $userdata['selectpanel'], "select");
    ...
    $locationproduct = select("durwinam_panel", "*", null, null, "count");
    ...
    sendmessage($from_id, $textbotlang['Admin']['Product']['serviceLocation'], $json_list_durwinam_panel, 'HTML');
    step('get_location', $from_id);
} elseif ($user['step'] == "get_location") {
    if ($text != '/all' && !rowExists("durwinam_panel", "name_panel", $text)) {
    ...
    $panel = select("durwinam_panel", "*", "name_panel", $text, "select");
    ...
    $userdata = json_decode($user['Processing_value'], true);
    $panel = select("durwinam_panel", "*", "name_panel", $userdata['Location'], "select");
    if ($panel['type'] == "Manualsale") {
        savedata("save", "Service_time", "0");
        savedata("save", "Volume_constraint", "0");
        sendmessage($from_id, $textbotlang['Admin']['Product']['getPrice'], $backadmin, 'HTML');
    ...
    $userdata = json_decode($user['Processing_value'], true);
    $panel = select("durwinam_panel", "*", "name_panel", $userdata['Location'], "select");
    if ($panel['type'] == "durwinam" || $panel['type'] == "marzneshin") {
    ...
    sendmessage($from_id, $textbotlang['Admin']['Product']['removeLocation'], $json_list_durwinam_panel, 'HTML');
    ...
    sendmessage($from_id, $textbotlang['Admin']['Product']['removeLocation'], $list_durwinam_panel_edit_product, 'HTML');
    ...
    $product = [];
    $panel = select("durwinam_panel", "*", "code_panel", $user['Processing_value_one'], "select");
    ...
    sendmessage($from_id, $textbotlang['Admin']['Product']['selectNewLocation'], $json_list_durwinam_panel, 'HTML');
    ...
    $panel = select("durwinam_panel", "*", "code_panel", $user['Processing_value_one']);
    ...
    sendmessage($from_id, $textbotlang['Admin']['Protocol']['removedProtocol'], $optionDurwinam, 'HTML');
    ...
    update("durwinam_panel", "MethodUsername", $text, "name_panel", $user['Processing_value']);
    $typepanel = select("durwinam_panel", "*", "name_panel", $user['Processing_value'], "select");
    ...
    update("durwinam_panel", "namecustom", $text, "name_panel", $user['Processing_value']);
    step('home', $from_id);
    $typepanel = select("durwinam_panel", "*", "name_panel", $user['Processing_value'], "select");
    ...
    sendmessage($from_id, $textbotlang['Admin']['managepanel']['getLoc'], $json_list_durwinam_panel, 'HTML');
    step('GetLocationEdit', $from_id);
} elseif ($user['step'] == "GetLocationEdit") {
    $durwinam_list_get = select("durwinam_panel", "*", "name_panel", $text, "select");
    if ($durwinam_list_get['type'] == "durwinam") {
        $Check_token = token_panel($durwinam_list_get['code_panel'], false);
        if (isset($Check_token['access_token'])) {
            $System_Stats = Get_System_Stats($text);
            if ($durwinam_list_get['version_panel'] == "1") {
            ...
            $__q41->bindValue(1, $durwinam_list_get['name_panel'], PDO::PARAM_STR);
            ...
            $__q42->bindValue(1, $durwinam_list_get['name_panel'], PDO::PARAM_STR);
            ...
            $Condition_durwinam = "";
            $text_durwinam = sprintf($textbotlang['Admin']['stats']['panelDurwinam'], $total_user, $active_users, $System_Stats['version'], $mem_total, $mem_used, $bandwidth, $ListSell, $ListSellSUM, $durwinam_list_get['agent']);
            if ($durwinam_list_get['version_panel'] == "1") {
                $text_durwinam = str_replace($textbotlang['keyboard']['durwinam'], $textbotlang['keyboard']['passargadPanel'], $text_durwinam);
            }
            sendmessage($from_id, $text_durwinam, $optionDurwinam, 'HTML');
        } elseif (isset($Check_token['detail']) && $Check_token['detail'] == "Incorrect username or password") {
            $text_durwinam = $textbotlang['Admin']['managepanel']['invalidCredentials'];
            sendmessage($from_id, $text_durwinam, $optionDurwinam, 'HTML');
        } else {
            $text_durwinam = $textbotlang['Admin']['managepanel']['errorStatusPanel'] . json_encode($Check_token);
            sendmessage($from_id, $text_durwinam, $optionDurwinam, 'HTML');
        }
    } elseif ($durwinam_list_get['type'] == "x-ui_single") {
        $status_server = status_server_xui($durwinam_list_get);
    ...
    } elseif ($durwinam_list_get['type'] == "durwinam_agent") {
        sendmessage($from_id, $textbotlang['users']['selectoption'], $option_durwinam, 'HTML');
    } elseif ($durwinam_list_get['type'] == "alireza_single") {
        $x_ui_check_connect = login($durwinam_list_get['code_panel'], false);
        if ($x_ui_check_connect['success']) {
            ...
            $text_durwinam = $textbotlang['Admin']['managepanel']['invalidCredentials'];
            sendmessage($from_id, $text_durwinam, $optionalireza_single, 'HTML');
        } else {
            $text_durwinam = $textbotlang['Admin']['managepanel']['errorStatusPanel'] . sprintf($textbotlang['Admin']['errorReason2'], $x_ui_check_connect['errror']);
            sendmessage($from_id, $text_durwinam, $optionalireza_single, 'HTML');
        }
    } elseif ($durwinam_list_get['type'] == "hiddify") {
        $System_Stats = serverstatus($durwinam_list_get['name_panel']);
        if (!empty($System_Stats['status']) && $System_Stats['status'] != 200) {
            $text_durwinam = $textbotlang['Admin']['managepanel']['fetchErrorCode'] . $System_Stats['status'];
            sendmessage($from_id, $text_durwinam, $optionhiddfy, 'HTML');
        } elseif (!empty($System_Stats['error'])) {
            $text_durwinam = $textbotlang['Admin']['managepanel']['fetchError'] . $System_Stats['error'];
            sendmessage($from_id, $text_durwinam, $optionhiddfy, 'HTML');
        } else {
            ...
            if (isset($System_Stats['stats'])) {
                ...
                $bandwidth = formatBytes($System_Stats['outgoing_bandwidth'] + $System_Stats['incoming_bandwidth']);
                $text_durwinam = sprintf($textbotlang['Admin']['stats']['panelServer'], $mem_total, $mem_used, $durwinam_list_get['agent']);
                sendmessage($from_id, $text_durwinam, $optionhiddfy, 'HTML');
            } elseif (isset($System_Stats['message']) && $System_Stats['message'] == "Unathorized") {
                $text_durwinam = $textbotlang['Admin']['managepanel']['invalidUrl'];
                sendmessage($from_id, $text_durwinam, $optionhiddfy, 'HTML');
            } else {
                sendmessage($from_id, $textbotlang['Admin']['managepanel']['notConnected'], $optionhiddfy, 'HTML');
            }
        }
    } elseif ($durwinam_list_get['type'] == "Manualsale") {
        sendmessage($from_id, $textbotlang['Admin']['selectOption2'], $optionManualsale, 'HTML');
    } elseif ($durwinam_list_get['type'] == "marzneshin") {
        $Check_token = token_panelm($durwinam_list_get['code_panel']);
        if (isset($Check_token['access_token'])) {
            ...
            $__q43->bindValue(1, $durwinam_list_get['name_panel'], PDO::PARAM_STR);
            ...
            $__q44->bindValue(1, $durwinam_list_get['name_panel'], PDO::PARAM_STR);
            ...
            $Condition_durwinam = "";
            $text_durwinam = sprintf($textbotlang['Admin']['stats']['panelDurwinam2'], $total_user, $active_users, $ListSell, $ListSellSUM, $durwinam_list_get['agent']);
            sendmessage($from_id, $text_durwinam, $optionmarzneshin, 'HTML');
        } elseif (isset($Check_token['detail']) && $Check_token['detail'] == "Incorrect username or password") {
            $text_durwinam = $textbotlang['Admin']['managepanel']['invalidCredentials'];
            sendmessage($from_id, $text_durwinam, $optionDurwinam, 'HTML');
        } else {
    ...
    } elseif ($durwinam_list_get['type'] == "WGDashboard") {
        sendmessage($from_id, $textbotlang['users']['selectoption'], $optionwg, 'HTML');
    } elseif ($durwinam_list_get['type'] == "s_ui") {
        sendmessage($from_id, $textbotlang['users']['selectoption'], $options_ui, 'HTML');
    } elseif ($durwinam_list_get['type'] == "ibsng") {
        $result = loginIBsng($durwinam_list_get['url_panel'], $durwinam_list_get['username_panel'], $durwinam_list_get['password_panel']);
        if ($result) {
            sendmessage($from_id, $result['msg'], $optionibsng, 'HTML');
        } else {
            sendmessage($from_id, $result['msg'], $optionibsng, 'HTML');
        }
    } elseif ($durwinam_list_get['type'] == "mikrotik") {
        $result = login_mikrotik($durwinam_list_get['url_panel'], $durwinam_list_get['username_panel'], $durwinam_list_get['password_panel']);
    ...
    } elseif ($durwinam_list_get['type'] == "rebecca") {
        $Check_connection = Get_System_Stats_rebecca($durwinam_list_get['name_panel']);
        if (empty($Check_connection['error']) && (empty($Check_connection['status']) || $Check_connection['status'] < 400)) {
            ...
            $ListSell->bindValue(1, $durwinam_list_get['name_panel'], PDO::PARAM_STR);
            ...
            $ListSellSum->bindValue(1, $durwinam_list_get['name_panel'], PDO::PARAM_STR);
            ...
            $text_durwinam = sprintf($textbotlang['Admin']['stats']['panelSales'], $ListSell, $ListSellSUM, $durwinam_list_get['agent']);
            sendmessage($from_id, $text_durwinam, $optionrebecca, 'HTML');
        } elseif (!empty($Check_connection['status']) && $Check_connection['status'] == 401) {
            $text_durwinam = $textbotlang['Admin']['managepanel']['invalidToken'];
            sendmessage($from_id, $text_durwinam, $optionrebecca, 'HTML');
        } else {
            $text_durwinam = $textbotlang['Admin']['managepanel']['errorStatusPanel'] . json_encode($Check_connection);
            sendmessage($from_id, $text_durwinam, $optionrebecca, 'HTML');
        }
    } else {
        sendmessage($from_id, $textbotlang['Admin']['selectOption2'], $optionDurwinam, 'HTML');
    }
    update("user", "Processing_value", $text, "id", $from_id);
    step('home', $from_id);
    ...
    update("user", "Processing_value", $text, "id", $from_id);
    update("durwinam_panel", "name_panel", $text, "name_panel", $user['Processing_value']);
    ...
    update("durwinam_panel", "url_panel", $text, "name_panel", $user['Processing_value']);
    update("durwinam_panel", "datelogin", null, "name_panel", $user['Processing_value']);
    step('home', $from_id);
    ...
    update("durwinam_panel", "agent", $text, "name_panel", $user['Processing_value']);
    step('home', $from_id);
    ...
    outtypepanel($typepanel['type'], $textbotlang['Admin']['managepanel']['changedUrlPanel']);
    update("durwinam_panel", "linksubx", $text, "name_panel", $user['Processing_value']);
    step('home', $from_id);
    ...
    update("durwinam_panel", "secret_code", $text, "name_panel", $user['Processing_value']);
    step('home', $from_id);
    ...
    update("durwinam_panel", "limit_panel", $text, "name_panel", $user['Processing_value']);
    step('home', $from_id);
    ...
    update("durwinam_panel", "time_usertest", $text, "name_panel", $user['Processing_value']);
    step('home', $from_id);
    ...
    $typepanel = select("durwinam_panel", "*", "name_panel", $user['Processing_value'], "select");
    outtypepanel($typepanel['type'], $textbotlang['Admin']['managepanel']['savedData']);
    update("durwinam_panel", "val_usertest", $text, "name_panel", $user['Processing_value']);
    step('home', $from_id);
    ...
    update("durwinam_panel", "inboundid", $text, "name_panel", $user['Processing_value']);
    step('home', $from_id);
    ...
    update("durwinam_panel", "username_panel", $text, "name_panel", $user['Processing_value']);
    update("durwinam_panel", "datelogin", null, "name_panel", $user['Processing_value']);
    step('home', $from_id);
    ...
    $durwinamprotocol = select("durwinam_panel", "*", "name_panel", $user['Processing_value'], "select");
    update("x_ui", "protocol", $text, "codepanel", $durwinamprotocol['code_panel']);
    step('home', $from_id);
    ...
    update("durwinam_panel", "password_panel", $text, "name_panel", $user['Processing_value']);
    update("durwinam_panel", "datelogin", null, "name_panel", $user['Processing_value']);
    step('home', $from_id);
    ...
    if ($text == $textbotlang['keyboard']['confirm']) {
        ...
        $durwinam = select("durwinam_panel", "*", "name_panel", $user['Processing_value'], "select");
        $stmt = $pdo->prepare("DELETE FROM durwinam_panel WHERE name_panel = :name_panel");
    ...
    sendmessage($from_id, $textbotlang['Admin']['Discount']['askProductLocation'], $json_list_durwinam_panel, 'HTML');
    step('getlocdiscount', $from_id);
    ...
    $userdata = json_decode($user['Processing_value'], true);
    $panel = select("durwinam_panel", "*", "name_panel", $userdata['name_panel'], "select");
    ...
    sendmessage($from_id, $textbotlang['Admin']['managepanel']['Inbound']['endInbound'], $optionDurwinam, 'HTML');
    step('home', $from_id);
    return;
    ...
    $durwinamcommission = select("affiliates", "*", null, null, "select");
    $keyboardcommission = json_encode([
        'inline_keyboard' => [
            [
                ['text' => $durwinamcommission['status_commission'], 'callback_data' => $durwinamcommission['status_commission']],
            ],
        ]
    ]);
    ...
    $durwinamDiscountaffiliates = select("affiliates", "*", null, null, "select");
    $keyboardDiscountaffiliates = json_encode([
        'inline_keyboard' => [
            [
                ['text' => $durwinamDiscountaffiliates['Discount'], 'callback_data' => $durwinamDiscountaffiliates['Discount']],
            ],
        ]
    ]);
    ...
    $durwinamstatusextra = select("shopSetting", "*", "Namevalue", "statusextra", "select")['value'];
    $durwinamdirectpay = select("shopSetting", "*", "Namevalue", "statusdirectpabuy", "select")['value'];
    ...
    $Bot_Status = json_encode([
        'inline_keyboard' => [
            ...
            [
                ['text' => $name_status_extra_Vloume, 'callback_data' => "editshops-extravolunme-$durwinamstatusextra"],
                ['text' => $textbotlang['Admin']['Status']['statusVolumeExtra'], 'callback_data' => "extravolunme"],
            ],
            [
                ['text' => $name_status_paydirect, 'callback_data' => "editshops-paydirect-$durwinamdirectpay"],
    ...
    $durwinam_list_get = select("durwinam_panel", "*", "name_panel", $nameloc['Service_location'], "select");
    ...
    $durwinam_list_get = select("durwinam_panel", "*", "name_panel", $invoice['Service_location'], "select");
...
if ($datain == "settimecornremove" && $adminrulecheck['rule'] == "administrator") {
    ...
    update("durwinam_panel", "Methodextend", $text, "name_panel", $user['Processing_value']);
    $typepanel = select("durwinam_panel", "*", "name_panel", $user['Processing_value'], "select");
    ...
    sendmessage($from_id, $textbotlang['Admin']['addorder']['stepThree'], $json_list_durwinam_panel, 'HTML');
    ...
    $durwinam_list_get = select("durwinam_panel", "*", "name_panel", $user['Processing_value_tow'], "select");
    ...
    if ($DataUserOut['status'] == "Unsuccessful") {
        ...
        if ($DataUserOut['username'] == null) {
            ...
            $texterros = sprintf($textbotlang['Admin']['reportgroup']['errorConfigCreateAdmin'], $DataUserOut['msg'], $from_id, $durwinam_list_get['name_panel']);
            if (strlen($setting['Channel_Report']) > 0) {
                telegram('sendmessage', [
                    'chat_id' => $setting['Channel_Report'],
    ...
    $stmt->execute();
    $output_config_link = $durwinam_list_get['sublink'] == "onsublink" ? $DataUserOut['subscription_url'] : "";
    $config = "";
    if ($durwinam_list_get['config'] == "onconfig" && is_array($DataUserOut['configs'])) {
    ...
    $textbotlang['textbot']['afterPay'] = $durwinam_list_get['type'] == "Manualsale" ? $textbotlang['textbot']['manual'] : $textbotlang['textbot']['afterPay'];
    $textbotlang['textbot']['afterPay'] = $durwinam_list_get['type'] == "WGDashboard" ? $textbotlang['textbot']['wgDashboard'] : $textbotlang['textbot']['afterPay'];
    $textbotlang['textbot']['afterPay'] = $durwinam_list_get['type'] == "ibsng" || $durwinam_list_get['type'] == "mikrotik" ? $textbotlang['textbot']['afterPayIbsng'] : $textbotlang['textbot']['afterPay'];
    ...
    $textcreatuser = str_replace('{location}', $durwinam_list_get['name_panel'], $textcreatuser);
    ...
    if ($durwinam_list_get['type'] == "Manualsale" || $durwinam_list_get['type'] == "ibsng" || $durwinam_list_get['type'] == "mikrotik") {
    ...
    sendMessageService($durwinam_list_get, $DataUserOut['configs'], $output_config_link, $DataUserOut['username'], $Shoppinginfo, $textcreatuser, $randomString, $user['Processing_value']);
    ...
    global $json_list_durwinam_panel_inbounds;
    ...
    $list_durwinam_panel_inbounds = [
        'keyboard' => [],
        'resize_keyboard' => true,
    ];
    foreach ($getinbounds as $button) {
        $list_durwinam_panel_inbounds['keyboard'][] = [
            ['text' => $button['tag']]
        ];
    }
    $list_durwinam_panel_inbounds['keyboard'][] = [
    ...
    $json_list_durwinam_panel_inbounds = json_encode($list_durwinam_panel_inbounds);
    update("user", "Processing_value_one", $text, "id", $from_id);
    sendmessage($from_id, $textbotlang['Admin']['managepanel']['Inbound']['getInbound'], $json_list_durwinam_panel_inbounds, 'HTML');
    step('getInbounddisable', $from_id);
} elseif ($user['step'] == "getInbounddisable") {
    sendmessage($from_id, $textbotlang['Admin']['managepanel']['inboundNameSaved'], $optionDurwinam, 'HTML');
    $textpro = "{$user['Processing_value_one']}*$text";
    update("durwinam_panel", "inbound_deactive", $textpro, "name_panel", $user['Processing_value']);
    step("home", $from_id);
    ...
    $panel = select("durwinam_panel", "*", "name_panel", $userdata['idpanel'], "select");
    for ($i = 0; $i < $userdata['count']; $i++) {
        ...
        $config = "";
        if ($durwinam_list_get['config'] == "onconfig" && is_array($dataoutput['configs'])) {
    ...
    sendmessage($from_id, $textbotlang['users']['selectoption'], $optionathdurwinam, 'HTML');
    $text_report = "";
    if (strlen($setting['Channel_Report']) > 0) {
    ...
    sendmessage($from_id, $textbotlang['Admin']['managepanel']['selectPanel'], $json_list_durwinam_panel, 'HTML');
    step('getlocoption', $from_id);
} elseif ($user['step'] == "getlocoption") {
    update("user", "Processing_value", $text, "id", $from_id);
    $typepanel = select("durwinam_panel", "*", "name_panel", $text, "select")['type'];
    if ($typepanel == "durwinam") {
        sendmessage($from_id, $textbotlang['users']['selectoption'], $optionathdurwinam, 'HTML');
    ...
    $panel = select("durwinam_panel", "*", "name_panel", $userdata['namepanel'], "select");
    if ($panel == false) {
    ...
    $panel = select("durwinam_panel", "*", "name_panel", $user['Processing_value'], "select");
    $listconfig = [];
    ...
    update("durwinam_panel", "priceChangeloc", $text, "name_panel", $user['Processing_value']);
    step('home', $from_id);
    ...
    $userdata = json_decode($user['Processing_value'], true);
    $typepanel = select("durwinam_panel", "*", "name_panel", $userdata['namepanel'], "select");
    ...
    update("durwinam_panel", "priceextravolume", $eextraprice, "name_panel", $userdata['namepanel']);
    ...
    $eextraprice = json_encode($eextraprice);
    update("durwinam_panel", "pricecustomvolume", $eextraprice, "name_panel", $userdata['namepanel']);
    update("user", "Processing_value", $userdata['namepanel'], "id", $from_id);
    step('home', $from_id);
    ...
    $eextraprice = json_encode($eextraprice);
    update("durwinam_panel", "priceextratime", $eextraprice, "name_panel", $userdata['namepanel']);
    update("user", "Processing_value", $userdata['namepanel'], "id", $from_id);
    step('home', $from_id);
    ...
    $eextraprice = json_encode($eextraprice);
    update("durwinam_panel", "pricecustomtime", $eextraprice, "name_panel", $userdata['namepanel']);
    update("user", "Processing_value", $userdata['namepanel'], "id", $from_id);
    step('home', $from_id);
    ...
    sendmessage($from_id, $textbotlang['Admin']['price']['askIncreasePanel'], $json_list_durwinam_panel, 'HTML');
    ...
    sendmessage($from_id, $textbotlang['Admin']['price']['askDecreasePanel'], $json_list_durwinam_panel, 'HTML');
    ...
    $eextraprice = json_encode($eextraprice);
    update("durwinam_panel", "mainvolume", $eextraprice, "name_panel", $userdata['namepanel']);
    update("user", "Processing_value", $userdata['namepanel'], "id", $from_id);
    step('home', $from_id);
    ...
    $eextraprice = json_encode($eextraprice);
    update("durwinam_panel", "maxvolume", $eextraprice, "name_panel", $userdata['namepanel']);
    update("user", "Processing_value", $userdata['namepanel'], "id", $from_id);
    step('home', $from_id);
    ...
    $eextraprice = json_encode($eextraprice);
    update("durwinam_panel", "maintime", $eextraprice, "name_panel", $userdata['namepanel']);
    update("user", "Processing_value", $userdata['namepanel'], "id", $from_id);
    step('home', $from_id);
    ...
    $eextraprice = json_encode($eextraprice);
    update("durwinam_panel", "maxtime", $eextraprice, "name_panel", $userdata['namepanel']);
    update("user", "Processing_value", $userdata['namepanel'], "id", $from_id);
    step('home', $from_id);
    ...
    update("durwinam_panel", "proxies", json_encode($userdata['service_ids']), "name_panel", $user['Processing_value']);
    step("home", $from_id);
    ...
    if ($panel['type'] == "durwinam") {
        if ($panel['version_panel'] == "1") {
            ...
            update("durwinam_panel", "inbounds", json_encode($DataUserOut['group_ids']), "name_panel", $user['Processing_value']);
            update("durwinam_panel", "proxies", json_encode($DataUserOut['proxy_settings'], true), "name_panel", $user['Processing_value']);
        } else {
            $DataUserOut = getuser($text, $user['Processing_value']);
            if (!empty($DataUserOut['error'])) {
                sendmessage($from_id, $DataUserOut['error'], null, 'HTML');
            ...
            update("durwinam_panel", "inbounds", json_encode($DataUserOut['inbounds']), "name_panel", $user['Processing_value']);
            update("durwinam_panel", "proxies", json_encode($DataUserOut['proxies'], true), "name_panel", $user['Processing_value']);
        }
        sendmessage($from_id, $textbotlang['Admin']['managepanel']['protocolSaved'], $optionDurwinam, 'HTML');
        ...
        $data = GetClientsS_UI($text, $panel['name_panel']); {
            ...
            update("durwinam_panel", "proxies", json_encode($servies, true), "name_panel", $user['Processing_value']);
        }
    } elseif ($panel['type'] == "ibsng" || $panel['type'] == "mikrotik") {
        update("durwinam_panel", "proxies", $text, "name_panel", $user['Processing_value']);
        ...
        update("durwinam_panel", "inbounds", json_encode($data['obj']['inboundIds']), "name_panel", $user['Processing_value']);
        ...
        update("durwinam_panel", "proxies", json_encode([$userdata['service_id']]), "name_panel", $user['Processing_value']);
    ...
    $durwinamstatus = select("durwinam_panel", "*", "name_panel", $user['Processing_value'], "select");
    $keyboardstatus = json_encode([
        'inline_keyboard' => [
            [
                ['text' => $durwinamstatus['status_extend'], 'callback_data' => $durwinamstatus['status_extend']],
            ],
        ]
    ]);
    sendmessage($from_id, $textbotlang['Admin']['Status']['activePanel'], $keyboardstatus, 'HTML');
} elseif ($datain == "on_extend") {
    update("durwinam_panel", "status_extend", "off_extend", "name_panel", $user['Processing_value']);
    $durwinamstatus = select("durwinam_panel", "*", "name_panel", $user['Processing_value'], "select");
    $keyboardstatus = json_encode([
        'inline_keyboard' => [
            [
    ...
    update("durwinam_panel", "status_extend", "on_extend", "name_panel", $user['Processing_value']);
    $durwinamstatus = select("durwinam_panel", "*", "name_panel", $user['Processing_value'], "select");
    $keyboardstatus = json_encode([
        'inline_keyboard' => [
            [
...
elseif ($text == $textbotlang['keyboard']['hidePanelForUser'] && $adminrulecheck['rule'] == "administrator") {
    ...
    update("durwinam_panel", "hide_user", $hideuserid, "name_panel", $user['Processing_value']);
    step('home', $from_id);
    ...
    $durwinam_list_get = select("durwinam_panel", "*", "code_panel", $user['Processing_value_one']);
    $datainbound = "";
    if ($durwinam_list_get['type'] == "durwinam") {
        $DataUserOut = getuser($text, $durwinam_list_get['name_panel']);
        if (!empty($DataUserOut['error'])) {
            sendmessage($from_id, $DataUserOut['error'], null, 'HTML');
            return;
        }
        ...
        $stmt->bindParam(':name_product', $user['Processing_value']);
        $stmt->bindParam(':Location', $durwinam_list_get['name_panel']);
        $stmt->bindParam(':agent', $user['Processing_value_tow']);
        $stmt->execute();
        $datainbound = json_encode($DataUserOut['inbounds']);
    } elseif ($durwinam_list_get['type'] == "marzneshin") {
        $userdata = json_decode(getuserm($text, $durwinam_list_get['name_panel'])['body'], true);
        if (isset($userdata['detail']) and $userdata['detail'] == "User not found") {
            sendmessage($from_id, $textbotlang['Admin']['managepanel']['userNotInPanel'], null, 'HTML');
            return;
        }
        $datainbound = json_encode($userdata['service_ids'], true);
    } elseif ($durwinam_list_get['type'] == "x-ui_single" || $durwinam_list_get['type'] == "alireza_single") {
        $datainbound = $text;
    } elseif ($durwinam_list_get['type'] == "s_ui") {
        $data = GetClientsS_UI($text, $durwinam_list_get['name_panel']);
        if (count($data) == 0) {
            sendmessage($from_id, $textbotlang['Admin']['managepanel']['userNotInPanel2'], $options_ui, 'HTML');
            return;
        }
    ...
    } elseif ($durwinam_list_get['type'] == "ibsng" || $durwinam_list_get['type'] == "mikrotik") {
        $datainbound = $text;
    } else {
    ...
    deletemessage($from_id, $message_id);
    $extend = $ManagePanel->extend($durwinam_list_get['Methodextend'], $prodcut['Volume_constraint'], $prodcut['Service_time'], $nameloc['username'], $prodcut['code_product'], $durwinam_list_get['code_panel']);
    if ($extend['status'] == false) {
        $extend['msg'] = json_encode($extend['msg']);
        $textreports = sprintf($textbotlang['Admin']['reportgroup']['errorRenewServiceAdmin'], $durwinam_list_get['name_panel'], $nameloc['username'], $extend['msg']);
...
if ($datain == "settimecornday" && $adminrulecheck['rule'] == "administrator") {
    ...
    if (!in_array($panel['subvip'], ['offsubvip', 'onsubvip'])) {
        update("durwinam_panel", "subvip", "offsubvip", "code_panel", $panel['code_panel']);
        $panel = select("durwinam_panel", "*", "code_panel", $panel['code_panel'], "select");
    }
    ...
    if (in_array($panel['type'], ['durwinam', "x-ui_single", "marzneshin"])) {
        $Bot_Status['inline_keyboard'][] = [
    ...
    if (in_array($panel['type'], ["durwinam"])) {
        $Bot_Status['inline_keyboard'][] = [
    ...
    if ($type == "stautsconfig") {
        ...
        update("durwinam_panel", "config", $valuenew, "code_panel", $code_panel);
    } elseif ($type == "sublink") {
        if ($value == "onsublink") {
            $valuenew = "offsublink";
        } else {
        ...
        update("durwinam_panel", "sublink", $valuenew, "code_panel", $code_panel);
        ...
        update("durwinam_panel", "status", $valuenew, "code_panel", $code_panel);
        ...
        update("durwinam_panel", "TestAccount", $valuenew, "code_panel", $code_panel);
        ...
        update("durwinam_panel", "conecton", $valuenew, "code_panel", $code_panel);
        ...
        update("durwinam_panel", "status_extend", $valuenew, "code_panel", $code_panel);
    } elseif ($type == "changeloc") {
        ...
        update("durwinam_panel", "changeloc", $valuenew, "code_panel", $code_panel);
        ...
        update("durwinam_panel", "inboundstatus", $valuenew, "code_panel", $code_panel);
    } elseif ($type == "subvip") {
        if ($value == "onsubvip") {
            $valuenew = "offsubvip";
        } else {
        ...
        update("durwinam_panel", "subvip", $valuenew, "code_panel", $code_panel);
        ...
        update("durwinam_panel", "customvolume", json_encode($customvlume), "code_panel", $code_panel);
        ...
        update("durwinam_panel", "on_hold_test", $valuenew, "code_panel", $code_panel);
    }
    $panel = select("durwinam_panel", "*", "code_panel", $code_panel, "select");
    $customvlume = json_decode($panel['customvolume'], true);
    $statusconfig = [
    ...
    update("durwinam_panel", "pricecustomvolume", $pricelist, null, null);
    ...
    update("durwinam_panel", "pricecustomtime", $pricelist, null, null);
    sendmessage($from_id, $textbotlang['Admin']['price']['savedGroup'], $keyboardadmin, 'HTML');
    step("home", $from_id);
    ...
    $id_user = $datagetr[1];
    savedata("clear", "id_user", $id_user);
    sendmessage($from_id, $textbotlang['Admin']['managepanel']['askHidePanelsAgent'], $json_list_durwinam_panel, 'HTML');
    ...
    sendmessage($from_id, $textbotlang['Admin']['gift']['askPanel'], $json_list_durwinam_panel, "html");
    step("getpanelgift", $from_id);
} elseif ($user['step'] == "getpanelgift") {
    $panel = select("durwinam_panel", "*", "name_panel", $text, "count");
    if ($panel == 0) {
    ...
    $durwinamporsant_one_buy = select("affiliates", "*", null, null, "select");
    $keyboardDiscountaffiliates = json_encode([
        'inline_keyboard' => [
            [
                ['text' => $durwinamporsant_one_buy['porsant_one_buy'], 'callback_data' => $durwinamporsant_one_buy['porsant_one_buy']],
            ],
        ]
    ]);
    ...
    sendmessage($from_id, $textbotlang['Admin']['managepanel']['askHidePanelProduct'], $json_list_durwinam_panel, 'HTML');
    ...
    foreach ($list_panel as $result) {
        $list_panel_keyboard['inline_keyboard'][] = [
            ['text' => $result['name_panel'], 'callback_data' => "set_panel_id_durwinam:{$result['id']}:{$panel['id']}"],
        ];
    }
    sendmessage($from_id, $textbotlang['Admin']['addorder']['selectPanel'], json_encode($list_panel_keyboard), 'HTML');
} elseif (preg_match('/set_panel_id_durwinam:(.*):(.*)/', $datain, $dataget)) {
    $panel_id_durwinam = $dataget[1];
    $panel_id = $dataget[2];
    deletemessage($from_id, $message_id);
    update("durwinam_panel", "inbounds", $panel_id_durwinam, "id", $panel_id);
    sendmessage($from_id, $textbotlang['Admin']['addorder']['panelSelected'], $option_durwinam, 'HTML');
