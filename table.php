...
try {
    ...
    $result = $pdo->query("SHOW TABLES LIKE 'durwinam_panel'");
    $table_exists = ($result->rowCount() > 0);

    if (!$table_exists) {
        $result = $pdo->query("CREATE TABLE durwinam_panel (
        id INT(6) UNSIGNED AUTO_INCREMENT PRIMARY KEY,
        ...
        ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE utf8mb4_unicode_ci");
        if (!$result) {
            echo "table durwinam_panel" . implode(' ', $pdo->errorInfo());
        }
    } else {
        ...
        addFieldToTable("durwinam_panel", "version_panel", "0", "VARCHAR(60)");
        addFieldToTable("durwinam_panel", "on_hold_test", "1", "VARCHAR(60)");
        addFieldToTable("durwinam_panel", "proxies", null, "TEXT");
        addFieldToTable("durwinam_panel", "inbounds", null, "TEXT");
        addFieldToTable("durwinam_panel", "customvolume", $VALUE, "TEXT");
        addFieldToTable("durwinam_panel", "subvip", "offsubvip", "VARCHAR(60)");
        addFieldToTable("durwinam_panel", "changeloc", "offchangeloc", "VARCHAR(60)");
        addFieldToTable("durwinam_panel", "hide_user", null, "TEXT");
        addFieldToTable("durwinam_panel", "status_extend", "on_extend", "VARCHAR(50)");
        addFieldToTable("durwinam_panel", "code_panel", null, "VARCHAR(50)");
        addFieldToTable("durwinam_panel", "priceextravolume", $valueprice, "VARCHAR(500)");
        addFieldToTable("durwinam_panel", "pricecustomvolume", $valueprice, "VARCHAR(500)");
        addFieldToTable("durwinam_panel", "pricecustomtime", $valueprice, "VARCHAR(500)");
        addFieldToTable("durwinam_panel", "priceextratime", $valueprice, "VARCHAR(500)");
        addFieldToTable("durwinam_panel", "priceChangeloc", "0", "VARCHAR(100)");
        addFieldToTable("durwinam_panel", "mainvolume", $valuemain, "VARCHAR(500)");
        addFieldToTable("durwinam_panel", "maxvolume", $valuemax, "VARCHAR(500)");
        addFieldToTable("durwinam_panel", "maintime", $valuemain, "VARCHAR(500)");
        addFieldToTable("durwinam_panel", "maxtime", $valuemax_time, "VARCHAR(500)");
        addFieldToTable("durwinam_panel", "MethodUsername", $textbotlang['keyboard']['numericIdRandom'], "VARCHAR(100)");
        addFieldToTable("durwinam_panel", "datelogin", null, "TEXT");
        addFieldToTable("durwinam_panel", "val_usertest", "100", "VARCHAR(50)");
        addFieldToTable("durwinam_panel", "time_usertest", "1", "VARCHAR(50)");
        addFieldToTable("durwinam_panel", "secret_code", null, "VARCHAR(200)");
        addFieldToTable("durwinam_panel", "inboundstatus", "offinbounddisable", "VARCHAR(50)");
        addFieldToTable("durwinam_panel", "inbound_deactive", "0", "VARCHAR(100)");
        addFieldToTable("durwinam_panel", "agent", "all", "VARCHAR(50)");
        addFieldToTable("durwinam_panel", "inboundid", "1", "VARCHAR(50)");
        addFieldToTable("durwinam_panel", "linksubx", null, "VARCHAR(200)");
        addFieldToTable("durwinam_panel", "conecton", "offconecton", "VARCHAR(100)");
        addFieldToTable("durwinam_panel", "type", "durwinam", "VARCHAR(50)");
        addFieldToTable("durwinam_panel", "Methodextend", $textbotlang['keyboard']['resetVolumeTime'], "VARCHAR(100)");
        addFieldToTable("durwinam_panel", "namecustom", "vpn", "VARCHAR(100)");
        addFieldToTable("durwinam_panel", "limit_panel", "unlimted", "VARCHAR(50)");
        addFieldToTable("durwinam_panel", "TestAccount", "ONTestAccount", "VARCHAR(50)");
        addFieldToTable("durwinam_panel", "status", "active", "VARCHAR(50)");
        addFieldToTable("durwinam_panel", "sublink", "onsublink", "VARCHAR(50)");
        addFieldToTable("durwinam_panel", "config", "offconfig", "VARCHAR(50)");
        addFieldToTable("durwinam_panel", "version_panel", "0", "VARCHAR(60)");
        $max_stmt = $pdo->prepare("SELECT MAX(CAST(SUBSTRING(code_panel, 3) AS UNSIGNED)) as max_num FROM durwinam_panel WHERE code_panel LIKE '7e%'");
        $max_stmt->execute();
        $max_row = $max_stmt->fetch(PDO::FETCH_ASSOC);
        $next_num = $max_row['max_num'] ? (int) $max_row['max_num'] + 1 : 15;
        $stmt = $pdo->prepare("SELECT id FROM durwinam_panel WHERE code_panel IS NULL OR code_panel = ''");
        $stmt->execute();
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            $code = '7e' . $next_num;
            $stmt = $pdo->prepare("UPDATE durwinam_panel SET code_panel = ? WHERE id = ?");
...
$pdo->query("ALTER TABLE durwinam_panel MODIFY password_panel TEXT COLLATE utf8mb4_bin");
$pdo->query("ALTER TABLE durwinam_panel MODIFY name_panel VARCHAR(255) COLLATE utf8mb4_bin");
...
$performanceIndexes = [
    ...
    ['durwinam_panel', 'idx_panel_name', '`name_panel`(150)'],
    ['durwinam_panel', 'idx_panel_code', '`code_panel`(100)'],
...
foreach ($performanceIndexes as $indexDef) {
    list($idxTable, $idxName, $idxCols) = $indexDef;
    try {
        $stmt = $pdo->prepare("SELECT COUNT(*) FROM information_schema.statistics WHERE table_schema = DATABASE() AND table_name = ? AND index_name = ?");
        $stmt->execute([$idxTable, $idxName]);
        if ((int) $stmt->fetchColumn() == 0) {
            $pdo->exec("ALTER TABLE `$idxTable` ADD INDEX `$idxName` ($idxCols)");
        }
    } catch (Exception $e) {
